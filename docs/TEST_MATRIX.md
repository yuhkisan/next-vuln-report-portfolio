...
| 4.1 | プロジェクトメニュー（名前変更/削除） | ⏳ | `app/api/projects/__tests__/route.test.ts` |
...
| 7.1 | プロジェクトメニュー（名前変更/削除） | ⏳ | `app/api/projects/__tests__/route.test.ts` |
...
- プロジェクトメニュー（名前変更/削除）の UI 挙動と永続化の確認
- ネットワークエラー / 予期しない失敗メッセージの表示（手動確認）
